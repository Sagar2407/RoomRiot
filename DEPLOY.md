...
The default launch config is **payments OFF** and a **free 3‑game night**
(Majority Report → Caption Court → Close Call). Everything is set in
`Dockerfile` / `render.yaml`; override with env vars (table at the bottom).
...
| `ROOM_RIOT_PLAYLIST` | `majority_report,caption_court,close_call` | The games a night runs. Comma‑separated from: `majority_report, bluff_bureau, caption_court, link_up, alibi_club, close_call`. |
