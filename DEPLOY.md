...
- `GET /metrics` — the §16 funnel (join → activation → completion, plus purchase
  metrics). **Token-protected** (see `ROOM_RIOT_METRICS_TOKEN`): pass
  `Authorization: Bearer <token>` or `?token=<token>`. In production it's locked
...
- **Rate limiting** (per client IP, via `trustProxy`): room create ≤ 20 / 5 min,
  ...
  metrics ≤ 30 / min. Reports (a gameplay socket action) are limited separately,
  ≤ 5 / 5 min per member. Static assets and gameplay moves are never rate limited.
- **Player limits enforced**: a room can't grow past its capacity (the largest
  game's max, 10), and a game whose minimum can't be met is skipped rather than
  started into a broken round.
- **Stale‑tap safety**: every game move names the game + phase it targets, so a
  tap that lands after the timer closes is rejected, never counted for the next
  round; the client retries a lost action under the same id (idempotent).
- **`/metrics` is token‑protected** and locked by default in production.
